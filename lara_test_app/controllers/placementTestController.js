  ...
  const getAllPlacementTests = async (req, res) => {
    try {
            ...
            include: [
                {
                    ...
                    include: [
                        {
                            model: Topic,
                            as: 'Topics', // Use the correct alias
        ...
        const formattedTests = placementTests.map(test => ({
            ...
            }).filter(topic => topic !== null) // Remove null values if any
        }));
    ...
    try {
        ...
        const topics = await db.PlacementTestTopic.findAll({
            ...
            include: [
                {
                    model: db.Topic,
                    as: 'Topics', // Ensure this matches the alias defined in the model
                    include: [
                        {
                            model: db.Subject,
                            as: 'subject', // Ensure this matches the alias defined in the model
        ...
        const topicDetails = topics.map(topicEntry => ({
            topic_id: topicEntry.Topics.topic_id,
            topic_name: topicEntry.Topics.name,
            subject_id: topicEntry.Topics.subject.subject_id,
            subject_name: topicEntry.Topics.subject.name
        }));
